package com.now.fitness.nowfitnessui.Object;

/**
 * Created by Romeric Heredia on 6/06/2017.
 */

public class MyWorkout {
    private int myWorkoutId;
    private String myWorkoutName;
    private int myWorkoutPlanId;

    public int getMyWorkoutId() {
        return myWorkoutId;
    }

    public void setMyWorkoutId(int myWorkoutId) {
        this.myWorkoutId = myWorkoutId;
    }

    public String getMyWorkoutName() {
        return myWorkoutName;
    }

    public void setMyWorkoutName(String myWorkoutName) {
        this.myWorkoutName = myWorkoutName;
    }

    public int getMyWorkoutPlanId() {
        return myWorkoutPlanId;
    }

    public void setMyWorkoutPlanId(int myWorkoutPlanId) {
        this.myWorkoutPlanId = myWorkoutPlanId;
    }
}
